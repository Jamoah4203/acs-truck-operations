...
export async function reportBreakdown(start:string,end:string){
 ...
 const rows=data||[];const group=(key:string,label:(r:any)=>string)=>{const m=new Map<string,{name:string,income:number,expense:number}>();for(const r of rows){const id=String(r[key]||'unassigned'),name=label(r)||'Unassigned',x=m.get(id)||{name,income:0,expense:0};x[r.direction==='income'?'income':'expense']+=Number(r.amount||0);m.set(id,x)}return[...m.values()].map(x=>({...x,net:x.income-x.expense})).sort((a,b)=>Math.abs(b.net)-Math.abs(a.net))};return{drivers:group('driver_id',r=>r.profiles?.full_name||'Unassigned'),vehicles:group('vehicle_id',r=>r.vehicles?.registration_number||'Unassigned'),categories:group('category_id',r=>r.transaction_categories?.name||'Unclassified')};
}
